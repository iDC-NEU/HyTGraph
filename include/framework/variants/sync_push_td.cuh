...
// ----------------------------------------------------------------
#ifndef HYBRID_SYNC_PUSH_TD_H
#define HYBRID_SYNC_PUSH_TD_H
                ...
                for (uint32_t i = 0 + tid; i < work_size; i += nthreads)
                {
                    ...
                    if (!enable_priority || app_inst.IsHighPriority(current_priority, node_buffer_datum.get_item(node)))
                    {
                        ...
                        if(node >= seg_snode && node < seg_enode){
                            if (pair.second){
                                uint64_t begin_edge = csr_graph.begin_edge(node),
                                        end_edge = csr_graph.end_edge(node);

                                if(!zcflag){
                                    begin_edge-=seg_sedge_csr;
                                    end_edge-=seg_sedge_csr;
                                }    
                                ...
                                // printf("%d %d %d\n",node, begin_edge, end_edge-begin_edge);
                                // if(node==10767||node==785951||node==828471||node==851670)
                                // for(int j=begin_edge;j<end_edge;j++){
